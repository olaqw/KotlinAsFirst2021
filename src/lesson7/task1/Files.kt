...
fun deleteMarked(inputName: String, outputName: String) {
    File(outputName).bufferedWriter().use { write ->
        File(inputName).forEachLine { l ->
            if (!l.startsWith("_")) write.write("$l\n")
        }
    }
}
...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val substring = substrings.toSet().toList()
    val res = mutableMapOf<String, Int>()
    val text = File(inputName).readText().lowercase()
    for (i in substring.indices) {
        if (!res.contains(substring[i])) res[substring[i]] = 0
        for (j in text.indices) if (text.startsWith(substring[i].lowercase(), j))
            res[substring[i]] = res[substring[i]]!! + 1
    }
    return res
}
...
fun centerFile(inputName: String, outputName: String) {
    ...
    val length = if (lines.isNotEmpty())
        lines.maxOf { it.length } else 0
    File(outputName).bufferedWriter().use { write ->
        lines.forEach { l ->
            val leftIndent = (length - l.length) / 2
            write.write(" ".repeat(leftIndent) + l + "\n")
        }
    }
}
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    try {
        ...
        if (list.size > 1) res.write(newLine.substring(0, newLine.lastIndex - 1)) else res.write("")
